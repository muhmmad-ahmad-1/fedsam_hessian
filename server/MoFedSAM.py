...
class MoFedSAM(Server):
    def __init__(self,dataset="CIFAR10",n_clients=10,training_rounds=50,learning_rate=1e-1,global_learning_rate = 0.5,batch_size=100,weight_decay=1e-3,optimizer="adam",loss_func="CELoss",rule="Drichlet",rule_arg=0.0,ub_sgm=0.0,aggr_scheme="FedAvg",aggr_args={},random_init = False,aggregation="default",grad_aggregator=True,lr_decay=0):
        super(MoFedSAM,self).__init__(dataset,n_clients,training_rounds,learning_rate,global_learning_rate,batch_size,weight_decay,optimizer,loss_func,rule,rule_arg,ub_sgm,aggr_scheme,aggr_args,random_init,aggregation,grad_aggregator,lr_decay)
    ...
    def train(self):
        ...
        with ThreadPoolExecutor() as executor:
            futures = [executor.submit(client.train, self.momentum) for i, client in enumerate(self.clients) if i in self.random_select]
