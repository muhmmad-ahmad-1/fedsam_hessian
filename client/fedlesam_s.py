...
class fedlesam_s(Client):
    def __init__(self,dataset,trn_x,trn_y,batch_size,loss_func,learning_rate,weight_decay,optimizer,max_norm,grad_aggregator = False,args = {"mu":0.0},epochs=3):
        super(fedlesam_s,self).__init__(dataset,trn_x,trn_y,batch_size,loss_func,learning_rate,weight_decay,optimizer,max_norm,grad_aggregator,args,epochs)
        self.deltas = None
    def init_optimizer(self, optimizer, learning_rate, weight_decay):
        ...
        self.optimizer = LESAM_S(self.model.parameters(),self.base_optimizer,rho=self.args.rho,adaptive=False)
